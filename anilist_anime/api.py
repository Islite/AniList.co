...
_H = {"User-Agent": "AnimeSearch/1.7.3 (ExteraGram)", "Content-Type": "application/json", "Accept": "application/json"}
...
def fetch_shiki_cover_rest(aid, cache=None):
    ...
    if cache is None:
        cache = {}
    ck = "shiki_cover_rest_%s" % aid
    if ck in cache:
        return cache[ck]
    try:
        r = requests.get(
            "https://shikimori.one/api/animes/%s" % aid,
            headers={"User-Agent": "AnimeSearch/1.7.3 (ExteraGram)", "Accept": "application/json"},
            timeout=6,
        )
