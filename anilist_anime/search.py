...
def send_card_and_desc(plugin, data, shiki, account, base, mapping, opts=None):
    ...
    # Карточка форматируется строго через исходный шаблон без изменений
...
def parse_anilist_media_list(media, mapping):
    ...
    for m in media:
        ...
        # Приоритет оригинального постера высокого разрешения из coverImage
        ci = m.get("coverImage") or {}
        cover_url = ci.get("extraLarge") or ci.get("large") or ci.get("medium") or ""
        if not cover_url and m.get("id") is not None:
            cover_url = "https://img.anili.st/media/%s" % m.get("id")
...
def parse_shikimori_media_list(media, mapping):
    ...
    for m in media:
        ...
        if isinstance(poster, dict):
            path = poster.get("originalUrl") or poster.get("mainUrl") or poster.get("original") or ""
        if not path:
            img = m.get("image")
...
def _row_from_id_data(data, shiki, mapping, prefer_shiki=False, prefer_anilist_cover=True):
    ...
    if data and isinstance(data, dict):
        ...
        if not al_cover and data.get("id") is not None:
